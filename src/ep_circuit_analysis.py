#!/usr/bin/env python3
"""
EP Circuit Analysis — Reproducing Zhao et al. Fig. 3c & 3d
============================================================

Analyzes |S21| transmission data from AWR Microwave Office for a
unidirectionally coupled LC resonator circuit near an exceptional point.

A perturbation capacitor Cε introduces reciprocal coupling, splitting
the transmission peak. The frequency splitting Δf should follow
Δf ∝ ε^(1/2) (square-root dependence), the hallmark of an EP sensor.
"""
...
C1_PF = 3200.0          # Main capacitor C1 in pF
KAPPA = 0.3125           # Coupling coefficient κ
F0_MHZ = 20.0            # Base resonant frequency in MHz
FREQ_RANGE = (10, 30)    # MHz range to search for peaks

# Cε sweep: 100 to 1600 pF in steps of 100
...
def load_data(filepath):
    """Load and parse the AWR Excel export.

    Returns frequency in MHz and a dict mapping Cε (pF) -> |S21| array.
    """
    df = pd.read_excel(filepath, header=0)

    # Frequency column — convert GHz to MHz
    freq_mhz = df.iloc[:, 0].values * 1000.0

    # Parse each |S21| column
    traces = {}
    for col in df.columns[1:]:
        # Column names like: |S(2,1)| : paper (Csensing = 100 )
        match = re.search(r'Csensing\s*=\s*(\d+)', col)
        if match:
            c_eps_pf = int(match.group(1))
            traces[c_eps_pf] = df[col].values
...
def find_two_peaks(freq_mhz, s21, freq_range=FREQ_RANGE):
    """Find the two dominant |S21| peaks using dual Lorentzian fitting.

    First locates approximate peaks via scipy.find_peaks, then refines
    with a dual Lorentzian fit on |S21|² for sub-grid accuracy.
    Returns (f_minus, f_plus) in MHz, where f_minus < f_plus.
    """
    mask = (freq_mhz >= freq_range[0]) & (freq_mhz <= freq_range[1])
    freq_sub = freq_mhz[mask]
    vals_sub = s21[mask]

    # --- Stage 1: rough peak locations via find_peaks ---
    freq_step = freq_sub[1] - freq_sub[0]
    min_dist = max(2, int(1.0 / freq_step))

    rough_peaks = None
    for prominence in [0.1, 0.05, 0.01, 0.005, 0.001, 0.0005]:
    ...
    # --- Stage 2: dual Lorentzian fit on |S21|² ---
    s21_sq = vals_sub ** 2
    a1_guess = s21_sq[rough_peaks[0]] * 1.0**2
    a2_guess = s21_sq[rough_peaks[1]] * 1.0**2
    ...
    try:
        ...
        f1_fit, f2_fit = sorted([popt[1], popt[4]])
        return f1_fit, f2_fit
    except (RuntimeError, ValueError):
        # Fall back to grid-based peaks if fitting fails
...
def main():
    data_path = Path('data/paper Csensing sweep (100-1600pf).xlsx')
    ...
    # ------------------------------------------------------------------
    print("=" * 60)
    print("EP Circuit Analysis — Zhao et al. Reproduction")
    print("=" * 60)
    ...
    for c_eps_pf in sorted(traces.keys()):
        if c_eps_pf == 0:
            continue  # skip zero-perturbation trace
    ...
    print(f"\nSuccessfully extracted peaks for {len(results_df)} / {len(CSENSING_VALUES)} traces")
    ...
    # Linear fit: log(Δf) = slope * log(ε) + intercept
    ...
    df_theory = delta_f_theory(eps_theory)
    ...
    ax.plot(np.log10(eps_theory), np.log10(df_theory), '--', color='#2ca02c',
            linewidth=2, label=r'Theory: $\Delta f = 2\sqrt{\kappa\varepsilon}\,f_0$')

    ax.set_xlabel(r'$\log_{10}(\varepsilon)$', fontsize=14)
    ax.set_ylabel(r'$\log_{10}(\Delta f\;[\mathrm{MHz}])$', fontsize=14)
    ax.set_title('Frequency Splitting vs. Perturbation Strength (Log-Log)', fontsize=14)
    ax.legend(fontsize=11)
    ax.grid(True, alpha=0.3)

    # Annotate slope
    ax.text(0.05, 0.95, f'Measured slope = {slope:.4f}\nTheoretical slope = 0.5',
            transform=ax.transAxes, fontsize=12, verticalalignment='top',
            bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.8))

    plt.tight_layout()
    loglog_path = output_dir / 'ep_loglog_splitting.png'
    ...
    cmap = plt.cm.viridis
    n_traces = len(sorted(traces.keys()))
    ...
    for i, c_eps_pf in enumerate(sorted_keys):
        ...
        if c_eps_pf == 0:
            # Scale down the zero-perturbation trace for display clarity
    ...
    ax.set_title(r'Transmission Spectra for Varying $C_\varepsilon$', fontsize=14)
    ax.set_xlim(10, 30)
    ax.legend(fontsize=9, ncol=2, loc='upper right')
    ax.grid(True, alpha=0.3)

    plt.tight_layout()
    waterfall_path = output_dir / 'ep_waterfall_traces.png'
    ...
    print(f"Deviation from theory:       {abs(slope - 0.5):.4f} ({abs(slope - 0.5)/0.5*100:.2f}%)")
    if abs(slope - 0.5) < 0.05:
        print("The measured slope is very close to 0.5, confirming the")
        print("square-root dependence Δf ∝ ε^(1/2) characteristic of an EP sensor.")
    else:
        print(f"The measured slope deviates from the expected 0.5 by {abs(slope - 0.5):.4f}.")
        print("This may indicate non-ideal effects in the circuit simulation.")

    # Save results table as CSV
    csv_path = output_dir / 'ep_splitting_results.csv'
